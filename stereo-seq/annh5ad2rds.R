...
infile <- argv$infile
outfile <- gsub("h5ad", "rds", infile)
metafile = paste0(strsplit(infile, "\\.")[[1]][1], ".obs/obs.csv")
...
object <- LoadH5Seurat(h5file, meta.data = FALSE, misc = FALSE)
object = AddMetaData(object, meta_data)
...
cell_coords = cell_coords[- c(which(is.na(cell_coords$x) | is.na(cell_coords$y))), ]
