...
input <- "/xdisk/mliang1/qqiu/project/multiomics-hypertension/subcluster/ec.pre_filter.scvi.strain_covar.gene_nb.hvg_2k.refined.merged.rds"
...
w1=which(CellChatDB[["interaction"]]$ligand == "H2-BI")
CellChatDB[["interaction"]] <- CellChatDB[["interaction"]][-w1,]
w2=which(CellChatDB[["interaction"]]$ligand == "H2-Ea-ps")
CellChatDB[["interaction"]] <- CellChatDB[["interaction"]][-w2,]


e <- new.env()
for(i in input_file){
  
  tissue = gsub("\\.(RNA|multiomics)\\.anno\\.rds", "", basename(i))
  
  seurat_object = readRDS(i)
  
  seurat_object$subclass_level2.1 = as.character(seurat_object$subclass_level2)
...
for (output_name in names(cellchat_list)) {
  ...
  output_name = output_name %>% gsub("\\.(RNA|multiomics)\\.anno\\.rds", "", .)
