        ...
        for (cell_type in cell_list) {
          ...
          fisher_test_df <- rbind(fisher_test_df, 
                                  data.frame(trait = trait, strain = strain, treatment = treatment, tissue = tissue, cell_type = cell_type, 
                                             ...
                                             p_value = fisher_test_result$p.value, gene_list = paste0(intersect(snp_gene_list, deg_list), collapse = ", "), NES = NES))
        }
      }
    }
  }
...
write.table(fisher_test_df, "trait.fisher.snp_gene.enrichment_results.out", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
